"""Worker handler for the `ingest` queue — M2b #5 PDF/DOCX ingest path.

Per [ADR-0021](../../docs/adr/0021-worker-http-callback-architecture.md):
    1. Read blob from `BLOB_STORAGE_DIR` + `payload.blob_storage_path`.
    2. Dispatch by `payload.content_type` to `parse_pdf` / `parse_docx`.
    3. On empty result → `mark_failed(ParseEmpty)`.
    4. SELECT current entry metadata (preserves title/category/tags/etc.).
    5. PUT to `INGEST_API_BASE_URL/api/ingest/<entry_id>` with the parsed
       body + preserved metadata + worker headers (sensitivity omitted
       so Node preserves the freshest value).
    6. On 2xx → `mark_done`. On 4xx/5xx/timeout → `mark_failed` with the
...
# Content-type → parser function dispatch. Keep the keys lower-case
# (HTTP content-types are case-insensitive but `app/api/ingest/upload`
# stores the value verbatim; we don't trust the case-folding to have
# happened upstream).
...
def make_handler(
    *,
    ...
    HTTP client, env paths) out of module-level state and lets tests
    substitute stubs trivially.
...
async def _run(
    ...
    *,
    ...
    parser = _PARSERS.get(content_type.lower())
    if parser is None:
        await _mark_failed_isolated(
            conn_factory=conn_factory,
            job_id=job.id,
    ...
    # ---------- Step 3: empty-result short-circuit ----------
    if not parsed_text.strip():
        await _mark_failed_isolated(
            conn_factory=conn_factory,
            job_id=job.id,
            worker_id=worker_id,
            error=f"parser returned empty text for {content_type} (image-only or empty doc?)",
            error_class=WorkerErrorClass.ParseEmpty,
        )
        return

    # ---------- Step 4: SELECT current entry metadata ----------
