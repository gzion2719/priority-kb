...
- **M2a item 2 — rename `audit_log.kind` from `"ingest"` to `"agent_ingest"` + add prompt_hash plumbing.** The M2a item 4 `POST /api/ingest` scaffold uses `kind:"ingest"` (does not match the `agent_%` CHECK) because the route is a structured-body endpoint, not an agent call. When M2a item 2 lands `prompts/ingestion-agent.md`, the chat-driven path that *invokes* the agent must (a) compute SHA-256 of the prompt file, (b) call the route with that hash, (c) the route writes `kind:"agent_ingest"` + `prompt_hash` on the audit row (the CHECK then enforces non-null). Don't forget to update `lib/ingest.test.ts` happy-path audit assertions when the rename lands.
