...
- [ ] Eval runner (`npm run eval` or `pytest evals/`) reports recall@5, citation precision; CI runs it on PR — **recall@5 leg shipped** 2026-05-27 ([evals/run.ts](../evals/run.ts) + [evals/run-adapter.ts](../evals/run-adapter.ts) + [evals/lib.ts](../evals/lib.ts) + [evals/schema.ts](../evals/schema.ts)); `pipelineAdapter` wired to `lib/retrieval-eval.ts::evalRetrieve` per ADR-0012 §7 produces real recall@5 numbers when run against local Postgres with seeded entries. **citation_precision leg pending** `evalRetrieveWithSynth` (BACKLOG; design choice between live-Anthropic-opt-in and an eval-specific stub that cites `reranked_ids[0]`); reports `skipped` for measured cases until then. CI integration (`npm run eval` job) is BACKLOG (needs Postgres + seed in CI).
