...
### Acceptance
On the golden eval set, retrieval recall@5 ≥ 0.8 and citation precision ≥ 0.9. Three Priority questions answered convincingly with citations. Degraded mode demoed by killing the Voyage env var. **Acceptance gating:** the metric bar is currently unmeasurable — golden-set `phase: ready` flip is gated on M2a item 8 (3 synthetic-fixture entries seeded per [ADR-0011 Amendment 2026-05-27](adr/0011-repo-visibility.md#amendment-2026-05-27--revert-trigger-event-gated-to-production-stage-transition)). Implementation (items 1-4 + 5 + 8) is complete; acceptance is Phase-B-gated. Production-stage real-data acceptance measurement is a separate gate that fires post-transition.
