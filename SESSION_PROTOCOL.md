...
**Platform-runtime-matching sub-clause (codified 2026-05-27).** Extension of Verification-layer-matching to the platform-runtime axis. When the focus touches a runtime layer whose behavior varies by platform — async event loop (`asyncio.run`, `psycopg.AsyncConnection.connect`, `WindowsSelectorEventLoopPolicy` vs `ProactorEventLoop`), native binary extension (C-extension wheels, Tesseract, ffmpeg), OS-specific API (Win32, Unix signals, file-locking semantics), or any code path that branches on `sys.platform` / `os.name` — the gate MUST include at least one test surface that exercises the real runtime on the SAME platform as a deployment target (Linux server / Windows local dev / macOS local dev). Stubbing the runtime layer at unit-test time (monkeypatching `conn_factory`, mocking event loops, asserting on a `sys.platform` mock) is necessary for fast feedback but CANNOT be the only verification — the cross-platform behavior lives in the unstubbed call path. **Trigger:** Step 7 plan calls `asyncio.run`, instantiates `psycopg.AsyncConnection`/`asyncpg.connect`, touches `sys.platform`/`os.name` branching, or imports a native-binary library → plan names the gate that exercises the real layer on the matching platform (CI on `ubuntu-latest` for production-Linux behavior, manual smoke on Windows for local-dev-Windows behavior, or a `windows-latest` CI matrix runner where one exists). **Origin:** 2026-05-27 M2b #5 — worker shipped with `asyncio.run` calling `psycopg.AsyncConnection.connect` (Option Y handler path). `make py-check` green and CI green on Linux (default `SelectorEventLoop` is psycopg-compatible) but Windows local dev raised `psycopg.InterfaceError: Psycopg cannot use the 'ProactorEventLoop' to run in async mode` on the first manual smoke after the release merged to main. Hotfix shipped as [#310](https://github.com/gzion2719/priority-kb/pull/310) (`WindowsSelectorEventLoopPolicy` set at top of worker `main()` gated on `sys.platform == "win32"`); the verification-layer gap is that CI is Linux-only AND unit tests stub `conn_factory`, so no automated test surface ever exercised the real Windows event-loop interaction. The manual-smoke step in ADR-0021 §Verification was the only gate that fired; smoke is unreliable as a primary gate. Cross-ref: parent Verification-layer-matching (above); BACKLOG entry queued for `windows-latest` CI matrix runner ahead of the M5 hosting decision.
...
<a name="py-sandbox-dep-parity"></a>
**Sandbox Python-dep parity (no library-stub overrides).** Before relying on `make py-check` in the sandbox, install the project's pinned Python deps (`pip install -e ".[dev]"` or the explicit pins from `pyproject.toml [project.dependencies]`). Do NOT add `ignore_missing_imports = true` overrides to `pyproject.toml [tool.mypy.overrides]` for libraries that ship typed stubs — the override masks real type errors that surface only in CI (where the libs are installed for real) and creates a sandbox-vs-CI drift class. **Trigger:** about to add a `[[tool.mypy.overrides]]` block OR a `# type: ignore[*]` comment on a third-party-lib call site in the sandbox → first install the lib and verify the error disappears against the real stubs. **Origin:** 2026-05-26 ADR-0019 M2b #3 session — sandbox override on `psycopg` + `pytest_asyncio` masked: (a) `psycopg.types.json.Jsonb` attribute-access errors (4 sites; the submodule isn't auto-re-exported, requires `from psycopg.types.json import Jsonb`), (b) `@pytest_asyncio.fixture` decorator-typing that needed no `# type: ignore` once the real package resolved. The masked errors surfaced only at CI gate time, costing 1 CI cycle. *Codified 2026-05-26.*
...
- **ADR-prose-vs-code-reconciliation sub-rule (codified 2026-05-27).** When an ADR or ADR amendment ships in the same PR as its implementation, before the Step 7b code-CR fires, `grep` the ADR section for every code-level identifier it names (function names, class names, constant names, type names, frozenset/dict literal names) and verify each appears in the implementation as described. ADR prose drafted at Step 7 plan time can drift from the final implementation if the implementation iterated against plan-CR findings; the drift surfaces at code-CR if there but author-side detection is a 30-second grep. **Trigger:** PR touches both `docs/adr/NNNN-*.md` and the file the ADR describes, in the same commit graph. **Origin:** 2026-05-27 M2b #6 worker-wiring session — Amendment A4 said the dispatch "swaps `_PARSERS: dict[str, ParserFn]` for `_PARSER_TYPES: frozenset[str]`" but the implementation kept `_PARSERS` (correct — dispatch needs the parser-fn lookup); code-CR M3 caught the drift. Cross-ref: Step 7 verify-before-finalize (parent rule, same author-side discipline applied to plan content rather than ADR-vs-code).
