...
**ROADMAP unchecked-box claim-link sub-rule (codified 2026-05-28).** `npm run check` chains the `tests/scripts/verify-roadmap-tickboxes.test.ts` mechanical floor, which flags any **unchecked `- [ ]` tickbox in `docs/ROADMAP.md` whose body contains a markdown link `[text](path)` to a resolving claim-shaped file** — extensions `.ts/.tsx/.mjs/.js/.yaml/.yml/.sql/.css/.ps1` (NOT `.md`, which is reference-shaped and exempt; NOT `.py`, which is not yet in `CLAIM_EXTS`). The guard reads such a link as "this box is shipped but still unticked" and fails the gate unless the box's first-line substring is in `KNOWN_PENDING_TICKBOXES`. **Therefore: when annotating a box you are deliberately keeping `[ ]` (partial-ship, deferred-leg, or for-contrast mention of an unrelated shipped file), cite the file as a backtick code span (`` `lib/scrub.ts` ``), never a markdown link** — the guard's documented non-goal #1 exempts backtick-only path references. Reserve `KNOWN_PENDING_TICKBOXES` allowlist entries for boxes that are genuinely shipped-but-intentionally-unticked (the "no rot" companion test requires every allowlist entry to actually trip as a candidate, so an unstarted box must NOT be allowlisted). **Trigger:** about to add a markdown link inside a `- [ ]` line in `docs/ROADMAP.md`. **Origin:** 2026-05-28 M2b tickbox-reconcile — the "genuinely unstarted" PII-scrub box (#8) cited `[lib/scrub.ts](../lib/scrub.ts)` for contrast; the `.ts` link resolved → guard failed `npm run check`; de-linking to a backtick span fixed it. One avoidable local gate round.
