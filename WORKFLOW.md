...
**No empty-diff `dev → main` release PR sub-rule.** Open the `dev → main` release PR **only when `git log origin/main..origin/dev` is non-empty**. The "Path A" pattern of opening a release PR while `dev == main` and trusting it to auto-grow on subsequent feature merges is fragile: the PR visibly looks empty for the whole window between creation and the first feature merge, which (a) reads as a broken handoff to the user, (b) becomes ambiguous when more than one feature PR is queued for the same release (which one does the empty release PR represent?). The cleaner shape: feature → dev merges first; then Claude opens the release PR against a non-empty diff and pairs it to the just-merged feature in the handoff. If a release PR was opened prematurely and the user has merged the feature in the meantime, verify with `gh pr view <num> --json additions,deletions,changedFiles` that auto-grow worked before declaring the pair complete. Codified 2026-05-17 after a session-closing handoff opened release PR #44 when `dev == main` for the ADR-0008 feature PR #43; #44 looked empty until #43 merged and the user pinged on the visible empty state.
