...
**Server-side safety net (`release-pr-autotitle.yml`).** Defense-in-depth for everything above: `.github/workflows/release-pr-autotitle.yml` runs on every `pull_request: opened|reopened|edited` event and, when `base == main && head == dev` and the title does not start with `release:`, calls `gh pr edit --title "release: dev → main"` server-side. This catches any title that slips through the prose rules — regardless of who opened the PR, which client they used, or whether `gh pr create` was denied. The handoff-quality title (with the scope summary) still comes from Claude per the rules above; the action only kicks in as a last-resort floor. If you find yourself relying on the action's rewrite instead of getting the title right at creation time, that's a signal the prose rules are leaking — fix them upstream rather than letting the floor catch you.
