...
**Parallel `dev → main` release PR sub-rule.** When opening a feature PR pair, open BOTH the `feat/* → dev` PR AND the `dev → main` release PR **in the same handoff** if at most one feature is currently queued for the release. The release PR is initially empty (`dev == main`) and auto-grows when the feature merges; one round-trip is saved per pair. When ≥2 feature PRs are stacked for one release, fall back to "open the release PR only after the previous feature merges" so the empty-state window stays unambiguous about which feature it represents.

Trigger: about to send a handoff with a `feat/* → dev` PR link.
- If `gh pr list --base main --state open` shows no open release PR AND no other feature PR is queued behind this one for the same release → open the release PR in parallel; both links appear in the same handoff.
- If a release PR is already open → reuse it (`gh pr view <num>`); do not open a duplicate.
- If another feature is queued behind this one → defer the release PR open until the prior feature merges.

If a release PR was opened prematurely and the user has merged the feature in the meantime, verify with `gh pr view <num> --json additions,deletions,changedFiles` that auto-grow worked before declaring the pair complete.

History: this rule was previously *no empty-diff `dev → main` release PR* (codified 2026-05-17 after release PR #44 looked empty until feature PR #43 merged). The conservative posture cost a round-trip on every pair while genuinely-stacked features are rare in practice; amended 2026-05-17 to the parallel-open default — the ambiguity case is still handled by the ≥2-features fallback.

**Claude never merges its own PRs.** `gh pr merge` (with or without `--auto`) is the user's click, not Claude's command. Claude runs the commit + push + `gh pr create`; the user clicks Merge in the GitHub UI (or runs the merge themselves). The wait between PR creation and merge is the **gate that forces second-look discipline** — it is the window in which Claude is supposed to think about whether another review fires (cross-ref `SESSION_PROTOCOL.md` Step 7b "Amplified covers review-induced plan changes"). Codified 2026-05-16 after Claude set `gh pr merge 35 --merge --auto`, CI passed in 90s, the auto-merge fired **before** Claude had pushed the code-CR fixes that the second Step 7b pass eventually surfaced — yielding a 2-PR fix slice (PR #36) where one cleaner PR would have sufficed. A future mechanical floor (PreToolUse hook on `gh pr merge`, parallel to the existing `gh pr create` precheck) is captured in BACKLOG.
