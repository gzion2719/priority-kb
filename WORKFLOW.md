...
**Retarget-banner sub-rule (codified 2026-05-26).** When BOTH legs of a stacked pair are opened in the same handoff (parent + child created before parent is merged), the post-parent-merge retarget instruction MUST appear as a **dedicated banner block at the top of the handoff message**, not as a checkbox inside a multi-step "Test plan" list. The user clicks "Merge" on the parent and then immediately on the child without re-reading the handoff; a retarget step that lived inside a checklist gets skipped. **Trigger:** any handoff that surfaces a `feat/A → dev` AND a stacked `feat/B → feat/A` PR pair in the same message. The retarget banner uses the exact shape:

> ⚠ **Stacked-PR retarget — DO BEFORE merging the child:** after PR #A merges to `dev`, run `gh pr edit <B> --base dev` and rebase `feat/B` onto `origin/dev`. If the child's merge UI fires while `baseRefName` still points at `feat/A`, the child's diff lands in the stale parent ref — recovery is a fresh PR from `feat/A` to `dev` (see Child-retarget-before-merge sub-rule for the failure-mode write-up).

**Origin:** 2026-05-26 M2b #3 ADR-0019 session — PRs #295 + #296 stacked-pair handoff named the retarget step as test-plan checkbox item 2; #296 was merged into `feat/m2b-3-jobs-queue` (not `dev`); recovery #297 required to bring #296's content over. 2nd recurrence of the #158-#161 bug class — per `feedback_prefer_mechanical_over_prose`, observation tier today; 3rd recurrence triggers a `gh pr merge` hook that blocks when `baseRefName` matches a `feat/*` pattern.

**Mechanical floor backing both stacked-PR sub-rules (codified 2026-05-30, 4th recurrence).** `.github/workflows/stacked-pr-baseref-check.yml` is the server-side gate that empirically catches a PR whose `baseRefName` is neither `dev` nor `main` AND whose base tip is an ancestor of `origin/dev` (= parent already merged) OR whose base ref no longer exists. The check name `stacked-pr-baseref-check` must be added to the `dev` branch-protection required-status-checks list to be enforcing. **Known race-condition gap closed by branch protection, not by the workflow:** when the parent merges to `dev` AFTER the child PR is opened AND the child receives no further push events, GitHub does not re-trigger this workflow — the child's last reported status stays green. Closing this race requires `required_status_checks.strict = true` ("Require branches to be up to date before merging") on the `dev` branch-protection rule. Both settings together close the surface. **Recurrence list:** PRs #158/#161 (2026-05-21), #295/#297 (2026-05-26), 3rd-strike Ceiling 2026-05-27 (deferred), #376/#377 (2026-05-30) — four prose-rule failures across nine days; per `feedback_prefer_mechanical_over_prose` the mechanical floor became mandatory.
...
**DB-integration-local-run-before-push sub-rule (codified 2026-05-30).** `npm run check` runs all `*.integration.test.ts` files with `describe.skip` when `DATABASE_URL` is unset — the skipped-locally / fail-loud-in-CI pattern (see `tests/entries.integration.test.ts`, `tests/ingest.integration.test.ts`, etc.). For any session that modifies a file the integration tests exercise — `lib/entries.ts`, `lib/ingest.ts`, `lib/retrieval-*.ts`, `lib/jobs.ts`, route handlers under `app/api/`, the page server components under `app/admin/` and `app/entries/` — run the matching integration suite against local docker Postgres BEFORE `git push`: `DATABASE_URL="postgres://postgres:postgres@localhost:5432/priority_kb" npx vitest run tests/<surface>.integration.test.ts`. The local gate's silence on these surfaces is not a green signal; it's a deferred CI cycle. **Trigger:** session-end pre-push self-check on any file that has a co-located `*.integration.test.ts`. **Origin:** 2026-05-30 M4 #1a session — `tests/entries.integration.test.ts` listEntriesForAdmin pagination test was the only signal that distinguished the cursor-= peek-ahead off-by-one (unit tests at `lib/entries.test.ts` had asserted the BUGGY behavior INTO existence — they passed). CI caught it; ~10 minutes wasted on a fixup commit + re-push + CI re-run. The same session's #1b and #1c both ran the integration suite locally and shipped CI-green first-try, proving the discipline pays for itself in one occurrence.
...
**Commit-subject-case sub-rule (codified 2026-05-28).** Commit subjects obey `commitlint.config.cjs` `subject-case` exactly like PR titles do — the subject must NOT be upper-case / pascal-case / start-case / sentence-case. A subject that LEADS with an identifier token (`ADR-0023 …`, `M2a …`, `PR #N …`, `OCR …`) trips `subject-case` because the first word is upper-case; rephrase to lead with a lowercase verb that keeps the identifier mid-subject: `add ADR-0023 …`, `tick M2a …`, `wire OCR …`. The `commit-msg` commitlint hook catches this, but the catch costs a re-commit round; pre-check the subject's first word before committing. The same lens already applies to PR titles via `scripts/precheck-pr-title.mjs` — extend it mentally to the commit subject. **Trigger:** about to author any `git commit` subject that starts with a capitalized identifier rather than a lowercase verb. **Origin:** 2026-05-27 Ceiling ("run the precheck on commit subjects too") then 2026-05-28 recurrence — the ADR-0023 commit subject `docs(adr): ADR-0023 image processing …` was rejected by commitlint `subject-case` (the `ADR-0023` lead), forcing a re-commit as `docs(adr): add ADR-0023 …`. 2nd occurrence of the commit-subject-case class; per `feedback_prefer_mechanical_over_prose` the 3rd triggers a `prepare-commit-msg`/`PreToolUse` mechanical floor. **3rd occurrence 2026-06-10** — M4.5/E commit subject `feat(ui): WCAG AA contrast pass …` rejected by commitlint `subject-case` (the `WCAG` lead), re-committed as `add WCAG …`. The mechanical floor is now **MANDATORY** per the recurrence threshold — BACKLOG'd under §Architecture & Infra ("`git commit` subject-case PreToolUse floor").
